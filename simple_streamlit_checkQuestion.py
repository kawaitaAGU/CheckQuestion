...
3. 問題文の指示（例：「2つ選べ」）と、実際に正しい選択肢の数が一致しているかを厳密に確認すること。
   - 一致しない場合は、「問題の構造に誤りがある」と明記して報告してください。
4. GPTが判断した正解と、画像内の正解表示（解答：など）が一致しているかを確認すること。
   - 一致しない場合はその理由を明示し、「問題の正答表示に誤りがある」と報告してください。
5. 専門用語・表現に誤字・脱字・表記ミスがないかを確認すること。
6. 国家試験問題としての構成（選択肢の体裁、設問構造、選択肢の数など）が適切であるか簡潔に評価すること。
7. 解説がある場合は、その記述内容が医学的に妥当かどうかをチェックして簡潔に報告すること。
